...
import {Card,Modal,CloseButton,Nav,Row} from 'react-bootstrap';
import '../../css/Home.css';
...
function Foetel() {
    ...
    const [AddItem,setAdditem]=useState(["Bulgur","Saláta","Bugonya","Hideg-meleg zöldség","Áfonyás almagombóc","meleg zöldség","Püré","Puliszka","Sajtos galuska",
  "Lepénygombóc","Köles","Lilakáposzta","Gersli","Káposzta"]);
    const [kosartart,setkosartart]=useState([])
    const [showCart, setShowCart] = useState(false);
   const [baselenght,setbase]=useState(0);
  const [data,setdata]=useState([])

    useEffect(()=>{
        axios({
          method: 'get',
          url: 'https://hidden-coast-90775.herokuapp.com/foods/Foetel',
          responseType: 'json'
        })
          .then((response)=> {
       ...
       const ModalById=async(id)=>{
        axios({
            ...
            setAdditem(["Bulgur","Saláta","Bugonya","Hideg-meleg zöldség","Áfonyás almagombóc","meleg zöldség","Püré","Puliszka","Sajtos galuska",
            "Lepénygombóc","Köles","Lilakáposzta","Gersli","Káposzta"])

            for (let index = 0; index < data.length; index++) {
              if(data[index]._id===id){
                  setbase(data[index].info)
              }
    ...
    const AddToCart=async function(name,info,ar,id,t){
...
<button type="button" onClick={Descrease}  className="btn btn-outline-danger rounded-circle"><svg xmlns="http://www.w3.org/2000/svg" width="45" height="55" fill="currentColor" class="bi bi-dash" viewBox="0 0 16 16">
  <path d="M4 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 4 8z"/>
                           ...
                           <button className='btn btn-danger' onClick={()=>AddToCart(food.name,foodInfo,food.Price,food._id,food.Type)} style={{marginTop:"18px"}}>Hozzáadás</button>
