...
function Borok() {
    ...
    useEffect(()=>{
        axios({
          method: 'get',
          url: 'https://hidden-coast-90775.herokuapp.com/foods/Bor',
          responseType: 'json'
        })
          .then((response)=> {
  ...
  const DescreaseDL=()=>{
    ...
    const AddToCart=function(name,info,ar,afa){
      var items = JSON.parse(localStorage.getItem('cart')) || [];
      var item = items.find(item => item.name === name&&item.info.join(',')===info.join(',')&&item.dl===dl);
     if (count===0) {
...
<span style={{backgroundColor:"", width:"20px",height:"20px",borderRadius:"50%"}}>{kosartart.length}</span>
  ...
  <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
                  ...
                  <div className='d-flex bg-light justify-content-center'>
                    <button onClick={IncreaseDL} type="button" className="btn btn-outline-danger rounded-circle"><svg xmlns="http://www.w3.org/2000/svg" width="45" height="55" fill="currentColor" className="bi bi-plus" viewBox="0 0 16 16">
  <path d="M8 4a.5.5 0 0 1 .5.5v3h3a.5.5 0 0 1 0 1h-3v3a.5.5 0 0 1-1 0v-3h-3a.5.5 0 0 1 0-1h3v-3A.5.5 0 0 1 8 4z"/>
</svg></button>
                            <h3 style={{marginTop:"20px",marginLeft:"50px",marginRight:"50px"}}>{dl}/dl</h3>

<button type="button" onClick={DescreaseDL} className="btn btn-outline-danger rounded-circle"><svg xmlns="http://www.w3.org/2000/svg" width="45" height="55" fill="currentColor" class="bi bi-dash" viewBox="0 0 16 16">
...
<button type="button" onClick={Descrease}  className="btn btn-outline-danger rounded-circle"><svg xmlns="http://www.w3.org/2000/svg" width="45" height="55" fill="currentColor" class="bi bi-dash" viewBox="0 0 16 16">
  <path d="M4 8a.5.5 0 0 1 .5-.5h7a.5.5 0 0 1 0 1h-7A.5.5 0 0 1 4 8z"/>
                           ...
                           <button className='btn btn-danger' onClick={()=>AddToCart(food.name,foodInfo,food.Price,food.AFA)} style={{marginTop:"18px"}}>Hozzáadás</button>
